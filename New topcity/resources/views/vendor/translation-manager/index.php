...
<head>
    ...
    <title><?php echo __('translation_manager.name')?></title>
    ...
    <div class="container-fluid">
        <div class="navbar-header">
            ...
            <a href="<?php echo action('\Barryvdh\TranslationManager\Controller@getIndex') ?>" class="navbar-brand">
                <?php echo __('translation_manager.name')?>
            </a>
...
<!--    <p>Warning, translations are not visible until they are exported back to the app/lang file, using <code>php artisan translation:export</code> command or publish button.</p>-->
    ...
    <div class="alert alert-success success-publish" style="display:none;">
        <p><?php echo __('translation_manager.group_translate_add_to_file_done') . ' "' . __('translation_manager.' . $group) . '"' ?></p>
    </div>
...
<!--        </form>-->
        ...
        <?php endif; ?>
        <?php if(isset($group)) : ?>
            <form class="form-inline form-publish" method="POST" action="<?php echo action('\Barryvdh\TranslationManager\Controller@postPublish', $group) ?>" data-remote="true" role="form" data-confirm="Ви впевнені що ви хочете опублікувати группу '<?php echo $group ?>?">
                <input type="hidden" name="_token" value="<?php echo csrf_token(); ?>">
                <button type="submit" class="btn btn-info" data-disable-with="Publishing.." >Опублікувати переклади</button>
    ...
    <form role="form" method="POST" action="<?php echo action('\Barryvdh\TranslationManager\Controller@postAddGroup') ?>">
        <input type="hidden" name="_token" value="<?php echo csrf_token(); ?>">
        <div class="form-group">
            <p><?php echo __('translation_manager.group_select_help_text')?></p>
            <select name="group" id="group" class="form-control group-select">
                <?php foreach($groups as $key => $value): ?>
                <?php if($value == 'auth' || $value == 'passwords' || $value == 'validation' || $value == 'pagination' || $value == 'voyager'){ continue; } if($value == 'Choose a group'){$value = 'choose_group';}?>
                    <option value="<?php echo $key ?>"<?php echo $key == $group ? ' selected':'' ?>><?php  echo trans('translation_manager.'.$value) ?></option>
                <?php endforeach; ?>
            </select>
        </div>
        <!--<div class="form-group">
            <label><?php /*echo __('translation_manager.add_new_group_help_text')*/?></label>
            <input type="text" class="form-control" name="new-group" />
        </div>
        <div class="form-group">
            <input type="submit" class="btn btn-default" name="add-group" value="<?php /*echo __('translation_manager.add_new_group_button')*/?> " />
        </div>-->
    </form>
    <?php if($group): ?>
        <!--<form action="<?php /*echo action('\Barryvdh\TranslationManager\Controller@postAdd', array($group)) */?>" method="POST"  role="form">
            <input type="hidden" name="_token" value="<?php /*echo csrf_token(); */?>">
            <div class="form-group">
                <label><?php /*echo __('translation_manager.add_new_key_to_group_help_text')*/?></label>
                <textarea class="form-control" rows="3" name="keys" placeholder="Add 1 key per line, without the group prefix"></textarea>
            </div>
            <div class="form-group">
                <input type="submit" value="<?php /*echo __('translation_manager.add_keys')*/?>" class="btn btn-primary">
            </div>
        </form>-->
        ...
        <hr>
    <h4><?php echo __('translation_manager.total')?>: <?= $numTranslations ?>, <?php echo __('translation_manager.changed')?>: <?= $numChanged ?></h4>
        <table class="table">
            <thead>
            <tr>
                <th width="15%">Key</th>
