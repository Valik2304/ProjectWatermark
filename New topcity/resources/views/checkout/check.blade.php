...
<table style="width: 100%; max-width: 750px;table-layout: fixed">
    <tr>
        <td rowspan="2"><img src="../img/image1.png" alt="Logotype"></td>
        <td colspan="2">
            <b>Постачальник</b>
            <br>
    ...
    <tr>
        <td style="padding-top: 20px;" colspan="2">
            <b>Одержувач {{ $order_legal_person->organization_name }} </b> <br>
            {{ $order_legal_person->legal_address . ', ' . $order_legal_person->inn}}
            {{ $order_legal_person->last_name . ' ' . $order_legal_person->first_name . '  ' . $order_legal_person->patronymic_name  . ', '.$order_legal_person->email}}
            <br>
            тел. {{ $order_legal_person->phone }}
        </td>
    </tr>
    <tr>
        <td style="text-align: center; padding-top: 40px" colspan="3">
            <b>Рахунок-фактура № ТС-{{ $order_id }}</b>
        </td>
    </tr>
    <tr>
        <td style="text-align: center;" colspan="3">
            <b>від {{ Carbon\Carbon::parse($order_date)->format('d'). ' ' . getMonth(Carbon\Carbon::parse($order_date)->format('m')) . ' ' .Carbon\Carbon::parse($order_date)->format('Y') }} р.</b>
        </td>
    </tr>
    <tr>
        <td colspan="3">
            <table style="width: 100%; border-collapse: collapse;">
                ...
                <tr>
                    ...
                    <td style="text-align: right; border: 1px solid black;">
                        <b>{{ round((($order_total)-($order_total*0.2)),2) }}</b>
                    </td>
                </tr>
                <tr>
                    <td style="vertical-align: top;" colspan="4" rowspan="2">{{ digit_text($order_total,'uk',true) }}</td>
                    <td style="border: 1px solid black;">
                        <b>ПДВ:</b>
                    </td>
                    <td style="text-align: right; border: 1px solid black;">
                        <b>{{ round(($order_total*0.2),2) }}</b>
                    </td>
                </tr>
                <tr>
                    <td style="border: 1px solid black;">
    ...
    <tr>
        <td>Сплатити до {{ Carbon\Carbon::parse($order_date)->addDays(2)->format('d'). ' ' . getMonth(Carbon\Carbon::parse($order_date)->addDays(2)->format('m')) . ' ' .Carbon\Carbon::parse($order_date)->addDays(2)->format('Y') }} р.</td>
        <td>Виписав(ла):</td>
        <td>
            <b>Романюк В.П</b>
        </td>
    ...
    <tr >
        <td colspan="2"><img style="width: 100%" src="../img/image3.png" alt=""></td>
    </tr>
</table>
